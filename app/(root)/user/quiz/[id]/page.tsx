      ...
      <section className="max-w-3xl mx-auto p-6 mt-28 bg-white rounded-2xl shadow space-y-5">
        ...
        >
          ...
          >
            {/* Контент модалки */}
            <motion.div
              initial={{ opacity: 0, y: 20 }} // Начальная анимация (при открытии)
              animate={{ opacity: 1, y: 0 }} // Когда модалка открыта
              exit={{ opacity: 0, y: 20 }} // Анимация при закрытии (эффект исчезновения и сдвига)
              transition={{ duration: 0.3, ease: "easeOut" }} // Параметры анимации
            >
...
function MatchQuestion({
  ...
  const leftItems = useMemo<DraggableItem[]>(
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (active.id.toString().startsWith("r-")) {
